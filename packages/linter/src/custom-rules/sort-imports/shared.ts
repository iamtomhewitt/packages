...
function printSortedItems (sortedItems: any[], originalItems: any[], sourceCode: any) {
  ...
  const sorted = sortedItems
    .map((groups) =>
      groups
        .map((groupItems: any[]) => groupItems.map((item) => item.code).join(newline))
        .join(newline),
    )
    .join(newline + newline);

  const sortedLines = sorted.split(newline);

  const thirdPartyImports: any[] = [];
  const localImports: any[] = [];
  const localScssImports: any[] = [];
  let reactImport = '';
  ...
  sortedLines.forEach((line) => {
    ...
    const isLocalImport = line.includes('from \'./') || line.includes('from \'../') || line.includes('from \'.\'');
    const isLocalScssImport = (line.includes('\'./') || line.includes('\'../')) && line.includes('.scss');
    const isStyleImport = line.includes('.scss') || line.includes('.css');
    const isReactImport = line.includes('from \'react\'');
    const isThirdPartyImport = !isLocalImport && !isLocalScssImport && !isReactImport;

    if (isLocalImport) {
      localImports.push(line);
    }
    else if (isLocalScssImport || isStyleImport) {
      localScssImports.push(line);
    }
    else if (isThirdPartyImport) {
  ...
  if (reactImport !== '') {
    alphabeticallySorted += reactImport + newline;
  }

  if (thirdPartyImports.length > 0) {
    alphabeticallySorted += thirdPartyImports.sort().map(l => l).join(newline);
    alphabeticallySorted += newline;
  }

  if (localImports.length > 0) {
    alphabeticallySorted += newline;
    alphabeticallySorted += localImports.sort().map(l => l).join(newline);
    alphabeticallySorted += newline;
  }

  if (localScssImports.length > 0) {
    alphabeticallySorted += newline;
    alphabeticallySorted += localScssImports.sort().map(l => l).join(newline);
    alphabeticallySorted += newline;
  }

  alphabeticallySorted = alphabeticallySorted.trim();
  ...
  const flattened = flatMap(sortedItems, (groups: any[]) => [].concat(...groups));
  const lastSortedItem: any = flattened[flattened.length - 1];
  ...
  return alphabeticallySorted + maybeNewline;
}
...
function getImportExportItems (
  ...
  return chunk.map((node: any, nodeIndex: number) => {
    const lastLine =
      nodeIndex === 0
    ...
    const commentsAfter = sourceCode
      ...
      .filter((comment: any) => comment.loc.end.line === node.loc.end.line);
...
function printWithSortedSpecifiers (node: any, sourceCode: any, getSpecifiers: any) {
  ...
  const items = itemsResult.items.map((originalItem: any, index: number) => ({
    ...originalItem,
  ...
  const sorted = flatMap(sortedItems, (item: any, index: number) => {
    ...
    const nonBlankIndex = item.after.findIndex(
      (token: any) => !isNewline(token) && !isSpaces(token),
    );
...
function getSpecifierItems (tokens: any[]) {
  ...
  let current = makeEmptyItem() as any;
  ...
  switch (current.state) {
    ...
    case 'specifier': {
      const lastIdentifierIndex = findLastIndex(
        current.specifier,
        (token2: any) => isIdentifier(token2) || isKeyword(token2),
      );
      ...
      const newlineIndexRaw = after.findIndex((token2: any) => isNewline(token2));
      ...
      const multilineBlockCommentIndex = after.findIndex(
        (token2: any) => isBlockComment(token2) && hasNewline(token2.code),
      );
...
function getAllTokens (node: any, sourceCode: any) {
  ...
  return flatMap(tokens, (token: any, tokenIndex: number) => {
    ...
    return [
      newToken,
      ...flatMap(comments, (comment: any, commentIndex: number) => {
        const previous =
